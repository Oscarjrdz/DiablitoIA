...
export async function GET() {
  try {
    const [ackLogs, ackUnmatched, ackNoop, lastSend, lastPayload, lastImagePayload, unknownEvent, fromMeDropped, lastMsgReceived] = await Promise.all([
      redis.lrange('debug_ack_log', 0, 29),
      redis.lrange('debug_ack_unmatched', 0, 29),
      redis.lrange('debug_ack_noop', 0, 29),
    ...
    const parsedUnmatched = ackUnmatched.map(l => { try { return typeof l === 'string' ? JSON.parse(l) : l; } catch { return l; } });
    const parsedNoop = ackNoop.map(l => { try { return typeof l === 'string' ? JSON.parse(l) : l; } catch { return l; } });
