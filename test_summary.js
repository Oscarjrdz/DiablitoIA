...
async function main() {
    ...
    msgAdmin += `📅 ${displayDateAdm} •  ⏰ 19:10 hrs\n\n`;
    ...
    activeStores.forEach((s, i) => {
        ...
        msgAdmin += `${emojis[i % emojis.length]} *${s.name.replace(/prueba|p-\d+/gi, '').trim()}*\n`;
        msgAdmin += `   💰 ${fmt(s.v)}\n`;
        msgAdmin += `   🧾 ${s.t} tickets (Ut: ${ltStr})\n\n`;
    });
    
    msgAdmin += `-----------------\n`;
    msgAdmin += `📈 *RESUMEN TOTAL*\n`;
    msgAdmin += `💰 Ventas Netas:  ${fmt(totalV)}\n`;
    msgAdmin += `💳 Devoluciones:  ${fmt(totalR)}\n`;
    msgAdmin += `🧾 Total Tickets: ${totalT}\n`;
    msgAdmin += `🤖 Regs x Bot: ${botRegs}\n`;
    msgAdmin += `⚡ _El Diablito Intelligence_`;

    console.log("=== MENSAJE PARA GRUPO ===");
    console.log(msg);
    console.log("\n=== MENSAJE PARA ADMIN (TU WHATSAPP) ===");
    console.log(msgAdmin);
    
    // Simulate sending to 8116038195
    ...
    async function sendWhatsApp(to, body, cfg) {
        if (!cfg.wappInstance || !cfg.wappToken) {
            console.error("Missing config to send WP");
            return;
        }
        ...
        console.log(`Mensaje enviado por WP a ${to}`);
    }

    // Replace the time visually for the simulation
    const fake8Msg = msg.replace(/⏰ \d\d:\d\d hrs/, '⏰ 20:00 hrs').replace("19:10", "20:00");
    await sendWhatsApp('5218116038195@c.us', fake8Msg, cfg);
